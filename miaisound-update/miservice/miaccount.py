...
class MiAccount:
    ...
    async def login(self, sid):
        ...
        try:
            ...
            if resp['code'] != 0:
                ...
                resp = await self._serviceLogin('serviceLoginAuth2', data2)
                if resp['code'] == 70016:
                    print(resp['desc'])
                    return False
                elif resp['code'] == 0:
            ...
            gobal.set_value('userId',resp['userId'])
            gobal.set_value('serviceToken',serviceToken)
    ...
    async def mi_request(self, sid, url, data, headers, relogin=True):
        if  await self.login(sid):  # Ensure login
